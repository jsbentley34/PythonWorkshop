...
def optimize_holt(data: List[Tuple[int, float]], horizon):
    years, measurements = zip(*data)
    estimated_years = years + tuple(range(years[-1], years[-1] + horizon))

    def fit(chromosome: Chromosome) -> float:
        smoothing, _ = holt(measurements, chromosome[0], chromosome[1],
                            chromosome[2], chromosome[3], horizon=horizon)

        return mape(measurements, smoothing)

    solver = Solver()
    model = solver.minimize(fit, [(0, 1), (0, 1),
                                  (0, 100), (0, 100)])
    ...
    plt.title("Population, total - Benin")
    plt.legend()
...
def optimize_holt_winters(data, horizon):
    years, measurements = zip(*data)
    estimated_years = years + tuple(range(years[-1], years[-1] + horizon))

    def fit(chromosome: Chromosome) -> float:
        smoothing, _ = holt_winters(measurements, chromosome[0], chromosome[1],
                                    chromosome[2], 12, horizon)
    ...
    print("MAPE: %f" % fit(model))
    smoothing, forecast = holt_winters(measurements, model[0], model[1],
                                       model[2], 12, horizon)
    plt.plot(years, measurements, label="Population, total")
    plt.plot(estimated_years, smoothing + forecast, label="Smoothing")
    plt.title("Population, total - Benin")
...
if __name__ == "__main__":
    ...
    persistence_data = load_wb_xml(
        r"Persistence_to_last_grade_of_primary.xml", "WLD")
